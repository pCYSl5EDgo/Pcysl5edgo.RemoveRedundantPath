...
#pragma warning restore IDE0018
        ...
        private int ProcessLoop(ref int segmentCharCount, ref int textIndex, ref bool isPreviousSeparatorCanonical, int continueLength, uint separator, uint altSeparator, uint separatorDuplicate, uint dot, uint current, uint parent, bool preserveTrailingDots, int batchIndex)
        {
            ...
            if (continueLength != 0)
            {
                if (continueLength > 0)
                {
                    var cleared = BitSpan.ZeroHighBits(separator, textIndex);
            ...
            do
            {
                if (BitSpan.GetBit(any, textIndex))
                {
                    ...
                    else if (BitSpan.GetBit(separator, textIndex))
                    {
                        isPreviousSeparatorCanonical = false;
                        var cleared = BitSpan.ZeroHighBits(~separator, textIndex);
                        textIndex = loopUpperLimit - BitOperations.LeadingZeroCount(cleared) - 1;
                        continue;
                    }
                    else
                    {
                ...
                {
                    var separatorCleared = BitSpan.ZeroHighBits(separator, textIndex);
                    ...
                    if (separatorIndex < 0)
                    {
                        ...
                        {
                            var dotCleared = BitSpan.ZeroHighBits(~dot, textIndex);
